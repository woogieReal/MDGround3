...
describe("treeCRUD", () => {
  ...
  describe("addTreeToRoot", () => {
    test("1 depth 추가", () => {
      const targetTree = cloneDeep(NEW_TREE);
      targetTree.treePath = "";

      const res = addTreeToRoot(MOCK_TREE_DATA, targetTree);
  ...
  describe("removeTreeFromRoot", () => {
    test("1 depth 제거", () => {
      const targetTree = cloneDeep(DEPTH_1_TREE);

      const res = removeTreeFromRoot(MOCK_TREE_DATA, targetTree);
